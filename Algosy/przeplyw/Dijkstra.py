...
def Dijkstra(graph, source):
    ...
    for vertice in vertices_list:
        ...
        prev.append(0)
        prev_weight.append(float('inf'))
        unvisited_dist.append(float('inf'))
        unvisited.append(vertice)

    dist[starting_v] = 0
    prev_weight[starting_v] = 0
    ...
    while unvisited:
        ...
        for neighbor in neighbors_list:
            v_in = neighbor[1]
            v_in_index = v_in - 1
            exam_v_index = examined_v - 1

            alt = dist[exam_v_index] + return_weight(graph, examined_v, v_in)

            if (alt < dist[v_in_index]):
                dist[v_in_index] = alt
                prev[v_in_index] = examined_v
                prev_weight[v_in_index] = return_weight(graph, examined_v, v_in)

    return dist, prev, prev_weight

distances, closest_n, prev_w = Dijkstra(vers, 1)

print('Vertice:', [1, 2, 3, 4, 5])
print('Closest:', closest_n)
print('CWeight:', prev_w)
print('Total_w:', distances)