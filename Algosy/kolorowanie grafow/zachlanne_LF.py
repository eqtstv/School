...
def lf_sort_vertices(graph):
    v_quan = max(max(graph))
    v_degrees = []
    vertices = list(range(1, v_quan+1)) 

    for v in range(v_quan):
        v_degrees.append(len(check_neighbours(edges1, v+1)))

    lf_sorted_vertices = [vertices for _,vertices in sorted(zip(v_degrees,vertices), reverse=True)]

    return lf_sorted_vertices

def color_graph(graph):

    v_quan = max(max(graph))
    vertices = lf_sort_vertices(graph)
